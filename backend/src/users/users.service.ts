...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto, @User() user: IUser) {
    const { name, phoneNumber, email, password, address, role } = createUserDto;

    // Kiểm tra xem email đã tồn tại và đã bị xóa hay không
    const existingUser = await this.userModel
      .findOne({ email })
      .where({ isDeleted: true });
    if (existingUser) {
      throw new BadRequestException(
        `Tài khoản có email: ${email} đã bị xóa. Vui lòng sử dụng email khác hoặc khôi phục tài khoản.`,
      );
    }
    ...
    const isExistPhone = await this.userModel.findOne({ phoneNumber });
    if (isExistPhone) {
      throw new BadRequestException(
        `Số điện thoại: ${phoneNumber} đã tồn tại trên hệ thống. Vui lòng sử dụng số điện thoại khác.`,
      );
    }
  ...
  async register(user: RegisterUserDto) {
    const { name, phoneNumber, email, password, address } = user;
  ...
  async dashboardUser(month: number, year: number, qs: any) {
    const startDate = new Date(Date.UTC(year, month - 1, 1, 0, 0, 0));
    const endDate = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999));

    const dataCreate = await this.userModel.aggregate([
      {
        $match: {
          createdAt: {
            $gte: startDate,
    ...
    const dataDelete = await this.userModel.aggregate([
      {
        $match: {
          deletedAt: {
            $gte: startDate,
    ...
    return {
      month: month,
      year: year,
      totalCreated: dataCreate[0]?.totalCreated || 0,
      totalDeleted: dataDelete[0]?.totalDeleted || 0,
    };
  }
  ...
  async retryActive(data: RetryActiveDto) {
    ...
    this.mailerService.sendMail({
      to: user.email, 
      subject: 'Activate your account at @kingoffood', 
      template: "register",
      context: {
  ...
  async getListUser() {
    return await this.userModel.find().select('_id name').exec();
  }
}
